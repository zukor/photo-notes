...
function issueNotificationControls(){if(window.PhotoNotesNative)return `<div class="issue-notification-controls"><button type="button" class="btn secondary" data-issue-push="enable">Enable issue notifications</button><button type="button" class="btn secondary slim" data-issue-push="disable">Disable on this iPhone</button><p data-push-status role="status">Get notified when a repair is ready to test or needs your decision.</p></div>`;return `<div class="issue-notification-controls" style="color:#000;text-align:left"><button type="button" class="btn secondary" data-issue-push="enable">Enable issue notifications</button><button type="button" class="btn secondary slim" data-issue-push="disable">Disable on this device</button><p data-push-status role="status" style="color:#000">On iPhone, add Photo Notes to your Home Screen and open it there first.</p></div>`;}
document.addEventListener('click',async event=>{
  ...
  try{
    if(window.PhotoNotesNative){status.textContent=await window.PhotoNotesNative.notifications(button.dataset.issuePush);return;}
