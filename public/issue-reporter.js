...
function updateIssueDescriptionLabel(){
  const labels={bug_problem:'Describe the bug or problem',ui_improvement:'Describe the suggested UI improvement',feature_improvement:'Describe the suggested feature improvement idea',new_feature:'Describe the new feature idea'};
  const label=labels[document.getElementById('issueType')?.value]||labels.bug_problem;
  const heading=document.getElementById('issueDescriptionLabel');if(heading)heading.textContent=uiPushText(label);
...
function issueReporterMarkup(){return `<a id="issueUpdates" class="issue-updates" href="/?issues=1" hidden data-html2canvas-ignore="true"></a>    <div class="issue-modal" id="issueModal" hidden data-html2canvas-ignore="true">
      <div class="issue-dialog" role="dialog" aria-modal="true" aria-labelledby="issueTitle">
        ...
        <div class="issue-description-heading"><label id="issueDescriptionLabel" for="issueDescription">Describe the bug or problem</label><button class="btn secondary" id="issueRecord" type="button">Click to speak description</button></div>
