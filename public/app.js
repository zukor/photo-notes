...
async function loadCards(area, query = '', filters = {}) {
  ...
  if (state._focusCapture) {
    ...
    if (chk) { const card = chk.closest('.card'); if (card) { card.scrollIntoView({ behavior: 'smooth', block: 'center' }); card.style.outline = '3px solid #e8231a'; setTimeout(() => { card.style.outline = ''; }, 2500); } }
  }
}
...
function addOverlayItem(t) {
  ...
  if (t === 'rect') {
    ...
    item = { t: 'rect', x: 30, y: 30, w: 40, h: 30, color: '#e8231a', thickness: 0.6 };
  } else if (t === 'arrow') {
    item = { t: 'arrow', x: 25, y: 25, w: 40, h: 30, color: '#e8231a', thickness: 0.8, dir: 'se' };
  } else {
...
function drawOverlayItems() {
  ...
  editorOverlays.forEach((it, i) => {
    if (it.t === 'rect' || it.t === 'arrow') {
      ...
      box.style.cssText = `position:absolute;left:${it.x}%;top:${it.y}%;width:${it.w}%;height:${it.h}%;${border}box-sizing:border-box;cursor:move;touch-action:none;${i === editorSel ? 'outline:2px dashed #e8231a;outline-offset:2px;' : ''}`;
      box.dataset.i = i;
      if (it.t === 'arrow') {
      ...
      if (i === editorSel) {
        ...
        handle.style.cssText = 'position:absolute;right:-9px;bottom:-9px;width:20px;height:20px;background:#e8231a;border:2px solid #fff;border-radius:50%;cursor:nwse-resize;touch-action:none';
...
function renderStampCtl() {
  ...
  const colors = ['#ffffff', '#000000', '#e8231a', '#c41d15', '#f28c28', '#f2c200', '#1b7a3d'];
  if (it.t === 'rect' || it.t === 'arrow') {
    const isArrow = it.t === 'arrow';
    box.innerHTML = `
      <label style="margin-top:12px">Selected: ${isArrow ? 'Arrow' : 'Box / Rectangle'}</label>
      <div class="status">Drag the ${isArrow ? 'arrow' : 'box'} to move it. Drag the red corner dot to resize.</div>
      ...
      <div class="pill-group" id="ovColors">${colors.map(col => `<div class="pill" data-col="${col}" style="background:${col};width:34px;height:28px;${it.color === col ? 'outline:3px solid #e8231a;' : ''}"></div>`).join('')}
        <input type="color" id="ovColorPick" value="${/^#[0-9a-fA-F]{6}$/.test(it.color) ? it.color : '#e8231a'}" style="width:44px;height:32px;padding:0;border:1px solid #000;border-radius:6px" />
      </div>
      ...
      <button class="btn secondary slim" id="ovDelete" style="color:#c41d15;margin-top:8px">Delete This ${isArrow ? 'Arrow' : 'Box'}</button>`;
  ...
  box.innerHTML = `
    ...
    <button class="btn secondary slim" id="ovDelete" style="color:#c41d15;margin-top:8px">Delete This Item</button>`;
...
function drawCropBox() {
  ...
  ['tl', 'tr', 'bl', 'br'].forEach(corner => {
    ...
    hd.style.cssText = `position:absolute;${pos};width:22px;height:22px;background:#e8231a;border:2px solid #fff;border-radius:50%;cursor:nwse-resize;touch-action:none`;
  ...
  list.innerHTML = groups.map(g => `
    <article class="card document-card">
      ...
      <div class="row" style="margin-top:8px">
        ...
        <button class="btn secondary slim" data-id="${g.id}" data-del="1" style="color:#c41d15">Delete</button>
      </div>
    </article>`).join('');
  ...
  if (!photos.length) pbox.innerHTML = '<p class="status">No photos yet. Add at least one.</p>';
    ...
    <div class="card" style="padding:8px">
      ...
      <div class="row" style="margin-top:6px">
        ...
        <button class="btn secondary slim ewrphotodel" data-pid="${p.id}" style="color:#c41d15">Remove</button>
      </div>
    </div>`).join('');
