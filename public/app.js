...
function renderGroupPairPreview() {
  ...
  box.innerHTML = `<div class="formhead" style="margin-top:18px">Before &amp; After Evidence</div>
    ...
    ${pairs.map(({ before, after }) => `<article class="card before-after-preview">
      <div class="before-after-column"><strong>BEFORE</strong><img src="${photoSrc(before.photo_path)}" alt="Before photo">${photoLocationHtml(before)}<div>${esc(before.note || '(no caption)')}</div><div class="meta">${new Date(before.created_at).toLocaleDateString(uiLocale())}</div></div>
      <div class="before-after-column"><strong>AFTER</strong><img src="${photoSrc(after.photo_path)}" alt="After photo">${photoLocationHtml(after)}<div>${esc(after.note || '(no caption)')}</div><div class="meta">${new Date(after.created_at).toLocaleDateString(uiLocale())}</div></div>
    </article>`).join('')}`;
}
