...
function qualityBlock(idres, idfmt) {
  return `
    <details style="margin-top:8px">
      <summary style="cursor:pointer;font-weight:bold;color:var(--pn-text-000,#000)">Photo quality &amp; format</summary>
      ...
      <p class="status" style="color:var(--pn-text-000,#000);margin-top:6px">Your originals stay full-resolution on the server. These only change what gets exported. Standard JPEG is best for uploading to an AI.</p>
    </details>`;
}
...
const MY_ISSUE_STATUS={retest_requested:'Retest Requested',blocked:'Needs attention',new:'Received',reviewing:'Automatic Repair Review',fixing:'Working',testing:'Testing',ready_to_test:'Deployed - awaiting your confirmation',tester_confirmed:'Closed - you confirmed',resolved:'Resolved',wont_fix:'Closed'};
...
async function renderMyIssueReports(){
  ...
  const r=await api('/api/issues/mine');
  if(!box.isConnected)return;
  if(!r.ok){box.innerHTML='<p class="status">Your issue reports could not be loaded.</p>';return;}
  const rows=await r.json();
  if(!box.isConnected)return;
  box.innerHTML=rows.length?rows.map(i=>`<article class="card tester-issue-card" data-issue-closed="${['resolved','tester_confirmed','wont_fix'].includes(i.management_status)}"><div class="tester-issue-head"><strong>Issue #${i.id}: ${esc(i.page_name||'Photo Notes')}</strong><span class="badge issue-status-${esc(i.management_status||'new')}">${esc(i.management_status==='blocked'&&i.review_decision==='clarify'?'Clarification Requested':i.management_status==='ready_to_test'&&(!i.verification||!i.release_reference)?'Ready status needs verification':(['ui_improvement','feature_improvement','new_feature'].includes(i.issue_type)?(i.management_status==='wont_fix'?'Closed - No Change':i.review_decision==='clarify'?'Clarification Requested':i.review_decision==='implement'&&i.management_status==='new'?'Approved For Implementation':i.management_status==='new'?'Awaiting Sam Review':MY_ISSUE_STATUS[i.management_status]):MY_ISSUE_STATUS[i.management_status])||'Received')}</span></div><div class="meta">Reported ${new Date(i.created_at).toLocaleString(uiLocale())}</div><p>${esc(i.description)}</p>${i.management_status==='blocked'&&(i.review_decision==='clarify'||i.issue_type!=='bug_problem')?`<div class="issue-fix-summary"><strong>${i.review_decision==='clarify'?'Directions or Questions From Sam':'What stopped and what happens next'}</strong><p style="white-space:pre-wrap">${esc(i.blocked_reason||'Please add reproduction details.')}</p><label for="issueDetails-${i.id}">Additional details, if requested</label><textarea id="issueDetails-${i.id}" maxlength="5000"></textarea><button class="btn" data-issue-details="${i.id}">Send Details for Review</button></div>`:''}${i.management_status==='blocked'&&i.issue_type==='bug_problem'&&i.review_decision!=='clarify'?'<p>This report is waiting for Sam’s review. No additional details are requested from you right now.</p>':''}${i.management_status==='wont_fix'&&i.review_note?`<div class="issue-fix-summary"><strong>Reason For No Change</strong><p>${esc(i.review_note)}</p></div>`:''}${i.fix_summary?`<div class="issue-fix-summary"><strong>What changed</strong><span>${esc(i.fix_summary)}</span></div>`:''}${i.deployed_at&&i.management_status!=='retest_requested'?`<div class="meta">Deployed: ${new Date(i.deployed_at).toLocaleString(uiLocale())}</div>`:''}<details data-issue-history="${i.id}"><summary>Issue history</summary><div>Open to load history.</div></details>${i.release_reference?`<div class="meta">Release: ${esc(i.release_reference)}</div>`:''}${i.management_status==='retest_requested'||(i.management_status==='ready_to_test'&&i.verification&&i.release_reference)?`<div class="issue-retest"><strong>${i.management_status==='retest_requested'?'Please Retest':'How to retest'}</strong><p style="white-space:pre-wrap">${esc(i.retest_instructions||'Refresh Photo Notes and repeat the steps that caused the problem.')}</p><label for="retestNotes-${i.id}">Optional retest note</label><textarea id="retestNotes-${i.id}" placeholder="Tell us only if something is still wrong."></textarea><div class="issue-retest-actions"><button class="btn" type="button" data-retest-fixed="${i.id}">${i.management_status==='retest_requested'?'No Longer Happening':'Fixed on my device'}</button><button class="btn secondary" type="button" data-retest-broken="${i.id}">Still happening</button></div></div>`:i.tester_result?`<div class="meta">Your retest: ${i.tester_result==='fixed'?'Fixed':'Still happening'}${i.tester_notes?' — '+esc(i.tester_notes):''}</div>`:''}</article>`).join(''):'<p class="status">You have not submitted any issue reports yet.</p>';
  const empty=document.createElement('p');
  empty.className='status';empty.setAttribute('role','status');box.appendChild(empty);
  const applyFilter=()=>{
    let visible=0;
    box.querySelectorAll('.tester-issue-card').forEach(card=>{
      card.hidden=myIssueFilter!=='all'&&card.dataset.issueClosed!==(myIssueFilter==='closed'?'true':'false');
      if(!card.hidden)visible++;
    });
    empty.hidden=!rows.length||visible>0;
    empty.textContent=uiT(myIssueFilter==='closed'?'No closed issues.':'No open issues.');
  };
  filter.onchange=()=>{myIssueFilter=filter.value;applyFilter();};
  applyFilter();
...
function renderSavedDimsEditor(c) {
  ...
  body.innerHTML = `
    ...
    ${c.photo_path ? `<div class="photo-box"><img src="${capturePhotoSrc(c)}" alt="capture" style="display:block" /></div>` : ''}
    ${dimBlockHtml()}
...
function defectBadgeHtml(c) {
  ...
  if (c.defect_type === 'none') {
    return `<span class="defbadge" data-id="${c.id}" style="background:var(--pn-bg-444444,#444444)">${esc(label)}</span>`;
  }
...
async function showEvidence(id){
  ...
  }catch(e){toast('Photo details and history could not be loaded');}
}
...
function renderStampCtl() {
  ...
  if (it.t === 'rect' || it.t === 'arrow') {
    ...
    box.innerHTML = `
      ...
      <div class="pill-group" id="ovColors">${colors.map(col => `<div class="pill" data-col="${col}" style="background:${col};width:34px;height:28px;${it.color === col ? 'outline:3px solid var(--pn-border-1d4ed8,#1d4ed8);' : ''}"></div>`).join('')}
        <input type="color" id="ovColorPick" value="${/^#[0-9a-fA-F]{6}$/.test(it.color) ? it.color : '#ff0000'}" style="width:44px;height:32px;padding:0;border:1px solid var(--pn-border-000,#000);border-radius:6px" />
      </div>
      ...
      <button class="btn secondary slim" id="ovDelete" style="color:var(--pn-text-c1121f,#c1121f);margin-top:8px">Delete This ${isArrow ? 'Arrow' : 'Box'}</button>`;
  ...
  box.innerHTML = `
    ...
    <label style="margin-top:8px">Color</label>
    <div class="pill-group" id="ovColors">${colors.map(col => `<div class="pill" data-col="${col}" style="background:${col};width:34px;height:28px;${it.color === col ? 'outline:3px solid var(--pn-border-1d4ed8,#1d4ed8);' : ''}"></div>`).join('')}
      <input type="color" id="ovColorPick" value="${/^#[0-9a-fA-F]{6}$/.test(it.color) ? it.color : '#ffffff'}" style="width:44px;height:32px;padding:0;border:1px solid var(--pn-border-000,#000);border-radius:6px" />
    </div>
    ...
    <button class="btn secondary slim" id="ovDelete" style="color:var(--pn-text-c1121f,#c1121f);margin-top:8px">Delete This Item</button>`;
...
async function saveOverlays() {
  const capture = editorCapture, overlays = JSON.parse(JSON.stringify(editorOverlays));
  const btn = document.getElementById('stampSave');
  if(btn.disabled)return false;
  btn.disabled = true; btn.textContent = 'Saving...';
  try {
    const r = await api(`/api/captures/${capture.id}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ overlays }),
    });
    if(!r.ok)throw Error('Save failed');
    const saved=await r.json();
    if(!Array.isArray(saved.overlays)||saved.overlays.length!==overlays.length)throw Error('Save failed');
    capture.overlays = saved.overlays;
    state.imgv=Date.now();
    toast('Changes saved');return true;
  } catch(error) { toast('Save failed');return false; }
...
function mapPopupHtml(c) {
  ...
  return `<div style="max-width:210px">
    <div class="photo-title">${esc(c.photo_title||'Untitled photo')}</div>
    ${c.photo_path ? `<img src="${capturePhotoSrc(c)}" style="width:100%;border-radius:4px" />` : ''}
    ${badge ? `<div style="margin:4px 0">${badge}</div>` : ''}
    ${photoLocationHtml(c,'No location')}
    <div style="font-size:12px;color:var(--pn-text-000,#000)">${esc(note)}${(c.note || '').length > 120 ? '…' : ''}</div>
...
function zonePopupHtml(z, defc) {
  ...
  return `<div style="min-width:190px">
    <div style="font-weight:bold;color:var(--pn-text-000,#000)">${esc(z.name)}</div>
    <div style="font-size:12px;color:var(--pn-text-000,#000)">${z.zone_type === 'polygon' ? 'approx length ' : ''}${len}, ${area}, ${defc} defects</div>
...
function renderEwrThumbs() {
  ...
  box.innerHTML = ph.map((f, i) => `<div style="position:relative"><img src="${URL.createObjectURL(f)}" style="width:90px;height:90px;object-fit:cover;border-radius:6px;border:1px solid var(--pn-border-000,#000)" /><button class="ewrrm" data-i="${i}" style="position:absolute;top:-6px;right:-6px;background:var(--pn-bg-b3261e,#b3261e);color:var(--pn-text-fff,#fff);border:none;border-radius:50%;width:22px;height:22px;font-weight:bold">×</button></div>`).join('');
...
function renderEwrView(body, data) {
  ...
  body.innerHTML = `
    ...
    <div class="row" style="margin-top:18px">
      ...
      <button class="btn" id="ewrDelete" style="background:var(--pn-bg-b3261e,#b3261e)">Delete Record</button>
    </div>`;
  ...
  if (!photos.length) pbox.innerHTML = '<p class="status">No photos yet. Add at least one.</p>';
    ...
    <div class="card" style="padding:8px">
      ...
      <div class="row" style="margin-top:6px">
        ...
        <button class="btn secondary slim ewrphotodel" data-pid="${p.id}" style="color:var(--pn-text-c1121f,#c1121f)">Remove</button>
      </div>
    </div>`).join('');
  ...
  box.innerHTML = items.map((c, i) => `
    <div class="card">
      ...
      <div class="row" style="margin-top:8px">
        ...
        <button class="btn" data-i="${i}" data-rm="1" style="background:var(--pn-bg-b3261e,#b3261e)">Remove</button>
      </div>
    </div>`).join('');
  wireRotate(box);
