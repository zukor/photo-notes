...
(async()=>{
 ...
 const labels=await page.locator('#profileMenu > a, #profileMenu > button').allTextContents();assert.equal(labels.pop(),'Sign Out');assert.deepEqual(labels,[...labels].sort((a,b)=>a.localeCompare(b,'en',{sensitivity:'base'})));
 assert.equal(await page.locator('#profileMenu a[href="/admin?view=super"]').count(),role==='super'?1:0);
 assert.equal(await page.locator('#profileMenu a[href="/admin?view=admin"]').count(),role==='user'?0:1);
