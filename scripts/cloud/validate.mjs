import fs from 'node:fs';import {execFileSync as exec,spawnSync} from 'node:child_process';import {parseProposal,applyProposal,approved,treeDigest,out} from './core.mjs';
...
const docker=['run','--rm','--network','none','--cap-drop','ALL','--security-opt','no-new-privileges','--pids-limit','256','--memory','2g','--cpus','2','-v',`${process.cwd()}:/work:ro`,'-w','/work','node:22-bookworm'];
...
if(before.error||before.status!==1||!/fail [1-9]/.test(before.stdout+before.stderr))throw new Error('Regression test did not reproduce the original failure');
...
out('digest',treeDigest(id));out('passed','true');
