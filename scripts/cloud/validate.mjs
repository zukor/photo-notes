...
const testImage='mcr.microsoft.com/playwright:v1.63.0-noble';
const docker=['run','--rm','--init','--shm-size','256m','--user',`${process.getuid()}:${process.getgid()}`,'--network','none','--cap-drop','ALL','--security-opt','no-new-privileges','--pids-limit','256','--memory','2g','--cpus','2','-v',`${process.cwd()}:/work:ro`,'-w','/work',testImage];
exec('docker',['pull',testImage],{stdio:'inherit'});
