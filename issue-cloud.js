...
function registerCloud(app,{pool,requireAuth,requireAdmin,requireTestingQueueToken}) {
  ...
  app.get('/api/automation/cloud-worker',requireTestingQueueToken,async(req,res)=>{try{const state=(await pool.query('SELECT last_tick,last_error FROM issue_cloud_state WHERE id=1')).rows[0];res.json({...state,dispatch_configured:!!process.env.ISSUE_GITHUB_TOKEN,runner_enabled:process.env.ISSUE_CLOUD_RUNNER_ENABLED==='true'});}catch{res.status(503).json({error:'Worker starting'});}});
