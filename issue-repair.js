...
function registerIssueRepair(app,{pool,requireAuth,requireAdmin,requireTestingQueueToken,uploadDir}){
  ...
  app.post('/api/issues/:id/details',requireAuth,async(req,res)=>{const detail=String(req.body?.details||'').trim();if(!detail||detail.length>5000)return res.status(400).json({error:'Enter 1–5,000 characters'});let client;
    try{client=await pool.connect();await client.query('BEGIN');const {rows}=await client.query("UPDATE issue_reports SET reporter_details=$1,review_decision=NULL,reviewed_by=NULL,management_status=CASE WHEN issue_type='bug_problem' THEN 'blocked' ELSE 'new' END,blocked_reason=CASE WHEN issue_type='bug_problem' THEN 'The tester has replied. Review their answer and choose Retry Repair when ready.' ELSE NULL END,repair_claim_hash=NULL,repair_lease_until=NULL,updated_at=now() WHERE id=$2 AND user_id=$3 AND management_status='blocked' AND (issue_type<>'bug_problem' OR review_decision='clarify') RETURNING id",[detail,req.params.id,req.user.id]);if(!rows.length){await client.query('ROLLBACK');return res.status(404).json({error:'Issue is not waiting for details'});}
