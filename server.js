...
async function burnOverlays(buffer, width, height, overlays, c) {
  ...
  for (const it of overlays) {
    ...
    if (it.t === 'rect') {
      ...
      const rcol = /^#[0-9a-fA-F]{3,8}$/.test(it.color || '') ? it.color : '#e8231a';
    ...
    if (it.t === 'arrow') {
      ...
      const acol = /^#[0-9a-fA-F]{3,8}$/.test(it.color || '') ? it.color : '#e8231a';
