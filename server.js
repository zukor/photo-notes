...
async function currentPlan(userId,db=pool) {
  try {
    const { rows } = await db.query(`SELECT plan FROM users WHERE id = $1`, [userId]);
    return rows.length && rows[0].plan === 'pro' ? 'pro' : 'free';
  } catch { return 'free'; }
}
async function currentProduct(userId,db=pool){try{const r=(await db.query(`SELECT plan,pro_type FROM users WHERE id=$1`,[userId])).rows[0];return r&&r.plan==='pro'?normalizeProType(r.pro_type):'basic';}catch(e){return 'basic';}}
...
app.post('/api/captures', requireAuth, validateCaptureAccount, upload.single('photo'), async (req, res) => {
  let receipt;
  try {
    receipt=await beginCaptureReceipt(req,pool);
    if(receipt.replay){if(req.file)await fs.promises.unlink(req.file.path).catch(()=>{});return res.json(receipt.replay);}
    const db=receipt.db;
    const b = req.body || {};
    ...
    const isProUser = await currentPlan(req.user.id,db) === 'pro';
    if (isProUser) {
    ...
    const captureProduct=await currentProduct(req.user.id,db);
    ...
    const concreteContext=concreteProduct?ConcreteCapture.normalize(b):{concrete_phase:null,concrete_purpose:null,concrete_stage:null};
    const concreteStage=concreteContext.concrete_stage||(concreteProduct&&!b.concrete_phase&&concreteStages.includes(b.concrete_stage)?b.concrete_stage:null);
...
app.post('/api/concrete/captures/:id/ticket',requireAuth,requireConcrete,upload.single('photo'),async(req,res)=>{try{
  const id=Number(req.params.id),placement=(await pool.query(`SELECT * FROM captures WHERE id=$1 AND user_id=$2 AND photo_path IS NOT NULL`,[id,req.user.id])).rows[0];
  if(!placement||!req.file){if(req.file&&!placement)try{fs.unlinkSync(req.file.path)}catch(e){};return res.status(placement?400:404).json({error:placement?'ticket photo required':'placement photo not found'});}
  const dims=await imageDims(req.file.path),photoPath=`/uploads/${req.file.filename}`,note=ticketText(req.body.note,1000)||'Concrete batch ticket / specification',ticket=(await pool.query(`INSERT INTO captures(user_id,captured_by,photo_path,photo_width,photo_height,note,address,area_tags,kind,job_id,concrete_element,concrete_stage,concrete_mix,concrete_location) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'note',$9,$10,'batch_ticket',$11,$12) RETURNING *`,[req.user.id,req.user.name,photoPath,dims&&dims.w,dims&&dims.h,note,placement.address,placement.area_tags||[],placement.job_id,placement.concrete_element,placement.concrete_mix,placement.concrete_location])).rows[0],buffer=fs.readFileSync(req.file.path);
  await pool.query(`INSERT INTO capture_evidence(capture_id,user_id,original_sha256,original_bytes,original_name) VALUES($1,$2,$3,$4,$5)`,[ticket.id,req.user.id,crypto.createHash('sha256').update(buffer).digest('hex'),buffer.length,ticketText(req.file.originalname,255)]);
  await pool.query(`INSERT INTO concrete_ticket_links(user_id,placement_capture_id,ticket_capture_id,reference_type) VALUES($1,$2,$3,$4)`,[req.user.id,id,ticket.id,req.body.reference_type==='specification'?'specification':'batch_ticket']);
  await recordCaptureHistory(req.user.id,ticket.id,'captured',{photo:true,concrete_reference_for:id});logEvent(req.user.id,'concrete_ticket_link',{placement_capture_id:id});res.json({ok:true,ticket});
}catch(e){console.error('[concrete.ticket]',e);res.status(500).json({error:'ticket photo failed'});}});

function evidenceChecklist(checks){const complete=checks.filter(c=>c.complete).length;return {checks,complete,total:checks.length,ready:complete===checks.length,warnings:checks.filter(c=>!c.complete).map(c=>c.warning)};}
function concreteEvidenceChecklist(rows,links){const stages=new Set(rows.map(r=>r.concrete_stage).filter(Boolean)),hasDefect=stages.has('defect');return evidenceChecklist([
  {key:'baseline',label:'Existing condition or pre-pour photo',complete:stages.has('existing_condition')||stages.has('pre_pour'),warning:'Add an existing-condition or pre-pour photo.'},
  {key:'preparation',label:'Formwork or reinforcement photo',complete:stages.has('formwork')||stages.has('reinforcement'),warning:'Add a formwork or reinforcement photo.'},
  {key:'placement',label:'Placement or finishing photo',complete:stages.has('placement')||stages.has('finishing'),warning:'Add a placement or finishing photo.'},
  {key:'completion',label:'Completed-work photo',complete:stages.has('completed'),warning:'Add a completed-work photo.'},
  {key:'verification',label:'Verification photo',complete:stages.has('verification'),warning:'Add a verification photo.'},
  {key:'ticket',label:'Batch ticket or specification linked',complete:links.length>0,warning:'Link a batch-ticket or specification photo.'},
...
app.get('/api/concrete/report',requireAuth,requireConcrete,async(req,res)=>{try{
  ...
  for(const row of rows)row.footprints=footprints.filter(f=>f.capture_id===row.id);
  ...
  const completeness=concreteEvidenceChecklist(rows,links);
  if(!req.query.doc)return res.json({counts,photos:rows,completeness});
...
// Photo-first HOA tools: asset records, guided inspection routes, and property visits.
app.get('/api/hoa/assets',requireAuth,requireHoa,async(req,res)=>{try{const vals=[req.hoaCompany.id],where=['a.company_id=$1','a.active=true'];if(Number.isInteger(Number(req.query.community_id))){vals.push(Number(req.query.community_id));where.push(`a.community_id=$${vals.length}`);}res.json((await pool.query(`SELECT a.*,c.name community_name,cap.photo_path,(SELECT COUNT(*)::int FROM hoa_asset_photos ap WHERE ap.asset_id=a.id) photo_count FROM hoa_assets a JOIN hoa_communities c ON c.id=a.community_id LEFT JOIN captures cap ON cap.id=a.primary_capture_id WHERE ${where.join(' AND ')} ORDER BY c.name,a.name`,vals)).rows);}catch(e){console.error('[hoa.assets]',e);res.status(500).json({error:'assets failed'});}});
app.post('/api/hoa/assets',requireAuth,requireHoa,upload.single('photo'),async(req,res)=>{try{const communityId=Number(req.body.community_id);if(!await hoaOwnsCommunity(req.hoaCompany.id,communityId)){if(req.file)try{fs.unlinkSync(req.file.path)}catch(e){};return res.status(400).json({error:'community required'});}const name=ticketText(req.body.name,200);if(!name||!req.file)return res.status(400).json({error:'name and photo required'});const dims=await imageDims(req.file.path),photoPath=`/uploads/${req.file.filename}`,cap=(await pool.query(`INSERT INTO captures(user_id,captured_by,photo_path,photo_width,photo_height,note,address,area_tags,kind) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'note') RETURNING *`,[req.user.id,req.user.name,photoPath,dims&&dims.w,dims&&dims.h,ticketText(req.body.note,4000),ticketText(req.body.address,500),[ticketText(req.body.asset_type,100)||'Asset']])).rows[0];const buffer=fs.readFileSync(req.file.path),hash=crypto.createHash('sha256').update(buffer).digest('hex');await pool.query(`INSERT INTO capture_evidence(capture_id,user_id,original_sha256,original_bytes,original_name) VALUES($1,$2,$3,$4,$5)`,[cap.id,req.user.id,hash,buffer.length,ticketText(req.file.originalname,255)]);const asset=(await pool.query(`INSERT INTO hoa_assets(company_id,community_id,name,asset_type,location_description,condition,primary_capture_id) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,[req.hoaCompany.id,communityId,name,ticketText(req.body.asset_type,100)||'Other',ticketText(req.body.location_description,500),['good','fair','poor','critical','not_assessed'].includes(req.body.condition)?req.body.condition:'not_assessed',cap.id])).rows[0];await pool.query(`INSERT INTO hoa_asset_photos(asset_id,capture_id,photo_type) VALUES($1,$2,'identity')`,[asset.id,cap.id]);res.json(asset);}catch(e){console.error('[hoa.asset.create]',e);res.status(500).json({error:'asset failed'});}});
app.get('/api/hoa/assets/:id',requireAuth,requireHoa,async(req,res)=>{try{const id=Number(req.params.id),asset=(await pool.query(`SELECT a.*,c.name community_name FROM hoa_assets a JOIN hoa_communities c ON c.id=a.community_id WHERE a.id=$1 AND a.company_id=$2`,[id,req.hoaCompany.id])).rows[0];if(!asset)return res.status(404).json({error:'not found'});const photos=(await pool.query(`SELECT c.id,c.photo_path,c.photo_title,c.note,c.address,c.latitude,c.longitude,c.created_at,ap.photo_type FROM hoa_asset_photos ap JOIN captures c ON c.id=ap.capture_id WHERE ap.asset_id=$1 ORDER BY ap.created_at`,[id])).rows;res.json({asset,photos});}catch(e){res.status(500).json({error:'asset failed'});}});
app.post('/api/hoa/assets/:id/photos',requireAuth,requireHoa,upload.single('photo'),async(req,res)=>{try{const id=Number(req.params.id),asset=(await pool.query(`SELECT * FROM hoa_assets WHERE id=$1 AND company_id=$2`,[id,req.hoaCompany.id])).rows[0];if(!asset||!req.file)return res.status(asset?400:404).json({error:asset?'photo required':'not found'});const dims=await imageDims(req.file.path),photoPath=`/uploads/${req.file.filename}`,cap=(await pool.query(`INSERT INTO captures(user_id,captured_by,photo_path,photo_width,photo_height,note,address,area_tags,kind) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'note') RETURNING *`,[req.user.id,req.user.name,photoPath,dims&&dims.w,dims&&dims.h,ticketText(req.body.note,4000),ticketText(req.body.address,500),[asset.asset_type]])).rows[0];const buffer=fs.readFileSync(req.file.path);await pool.query(`INSERT INTO capture_evidence(capture_id,user_id,original_sha256,original_bytes,original_name) VALUES($1,$2,$3,$4,$5)`,[cap.id,req.user.id,crypto.createHash('sha256').update(buffer).digest('hex'),buffer.length,ticketText(req.file.originalname,255)]);await pool.query(`INSERT INTO hoa_asset_photos(asset_id,capture_id,photo_type) VALUES($1,$2,$3)`,[id,cap.id,['identity','condition','damage','repair','verification'].includes(req.body.photo_type)?req.body.photo_type:'condition']);await pool.query(`UPDATE hoa_assets SET condition=$1,updated_at=now() WHERE id=$2`,[['good','fair','poor','critical','not_assessed'].includes(req.body.condition)?req.body.condition:asset.condition,id]);res.json({ok:true});}catch(e){console.error('[hoa.asset.photo]',e);res.status(500).json({error:'photo failed'});}});

app.get('/api/hoa/routes',requireAuth,requireHoa,async(req,res)=>{try{res.json((await pool.query(`SELECT r.*,c.name community_name,COUNT(s.id)::int stop_count FROM hoa_inspection_routes r JOIN hoa_communities c ON c.id=r.community_id LEFT JOIN hoa_inspection_stops s ON s.route_id=r.id WHERE r.company_id=$1 AND r.active=true GROUP BY r.id,c.name ORDER BY c.name,r.name`,[req.hoaCompany.id])).rows);}catch(e){res.status(500).json({error:'routes failed'});}});
app.post('/api/hoa/routes',requireAuth,requireHoa,async(req,res)=>{const client=await pool.connect();try{const b=req.body||{},communityId=Number(b.community_id),name=ticketText(b.name,200),stops=Array.isArray(b.stops)?b.stops.slice(0,100):[];if(!name||!stops.length||!await hoaOwnsCommunity(req.hoaCompany.id,communityId))return res.status(400).json({error:'community, name, and stops required'});await client.query('BEGIN');const route=(await client.query(`INSERT INTO hoa_inspection_routes(company_id,community_id,name,instructions) VALUES($1,$2,$3,$4) RETURNING *`,[req.hoaCompany.id,communityId,name,ticketText(b.instructions,1000)])).rows[0];for(let x=0;x<stops.length;x++){const s=stops[x]||{},stopName=ticketText(s.name,200);if(!stopName)continue;const views=Array.isArray(s.required_views)?s.required_views.map(v=>ticketText(v,50)).filter(Boolean).slice(0,8):['overview'];await client.query(`INSERT INTO hoa_inspection_stops(route_id,asset_id,name,instructions,required_views,sort_order) VALUES($1,$2,$3,$4,$5,$6)`,[route.id,Number.isInteger(Number(s.asset_id))?Number(s.asset_id):null,stopName,ticketText(s.instructions,500),views.length?views:['overview'],x]);}await client.query('COMMIT');res.json(route);}catch(e){await client.query('ROLLBACK');console.error('[hoa.route.create]',e);res.status(500).json({error:'route failed'});}finally{client.release();}});
app.get('/api/hoa/routes/:id',requireAuth,requireHoa,async(req,res)=>{try{const route=(await pool.query(`SELECT r.*,c.name community_name FROM hoa_inspection_routes r JOIN hoa_communities c ON c.id=r.community_id WHERE r.id=$1 AND r.company_id=$2`,[Number(req.params.id),req.hoaCompany.id])).rows[0];if(!route)return res.status(404).json({error:'not found'});route.stops=(await pool.query(`SELECT s.*,a.name asset_name FROM hoa_inspection_stops s LEFT JOIN hoa_assets a ON a.id=s.asset_id WHERE s.route_id=$1 ORDER BY s.sort_order,s.id`,[route.id])).rows;res.json(route);}catch(e){res.status(500).json({error:'route failed'});}});
app.post('/api/hoa/visits',requireAuth,requireHoa,async(req,res)=>{const client=await pool.connect();try{const routeId=Number(req.body&&req.body.route_id),route=(await client.query(`SELECT * FROM hoa_inspection_routes WHERE id=$1 AND company_id=$2 AND active=true`,[routeId,req.hoaCompany.id])).rows[0];if(!route)return res.status(404).json({error:'route not found'});await client.query('BEGIN');const visit=(await client.query(`INSERT INTO hoa_property_visits(company_id,community_id,route_id,created_by) VALUES($1,$2,$3,$4) RETURNING *`,[req.hoaCompany.id,route.community_id,route.id,req.user.id])).rows[0];await client.query(`INSERT INTO hoa_visit_stops(visit_id,inspection_stop_id,name,instructions,required_views,sort_order) SELECT $1,id,name,instructions,required_views,sort_order FROM hoa_inspection_stops WHERE route_id=$2 ORDER BY sort_order,id`,[visit.id,route.id]);await client.query('COMMIT');res.json(visit);}catch(e){await client.query('ROLLBACK');res.status(500).json({error:'visit failed'});}finally{client.release();}});
app.get('/api/hoa/visits',requireAuth,requireHoa,async(req,res)=>{try{res.json((await pool.query(`SELECT v.*,c.name community_name,r.name route_name,COUNT(s.id)::int total_stops,COUNT(s.id) FILTER(WHERE s.status='complete')::int completed_stops FROM hoa_property_visits v JOIN hoa_communities c ON c.id=v.community_id LEFT JOIN hoa_inspection_routes r ON r.id=v.route_id LEFT JOIN hoa_visit_stops s ON s.visit_id=v.id WHERE v.company_id=$1 GROUP BY v.id,c.name,r.name ORDER BY v.started_at DESC LIMIT 100`,[req.hoaCompany.id])).rows);}catch(e){res.status(500).json({error:'visits failed'});}});
app.get('/api/hoa/visits/:id',requireAuth,requireHoa,async(req,res)=>{try{const visit=(await pool.query(`SELECT v.*,c.name community_name,r.name route_name FROM hoa_property_visits v JOIN hoa_communities c ON c.id=v.community_id LEFT JOIN hoa_inspection_routes r ON r.id=v.route_id WHERE v.id=$1 AND v.company_id=$2`,[Number(req.params.id),req.hoaCompany.id])).rows[0];if(!visit)return res.status(404).json({error:'not found'});visit.stops=(await pool.query(`SELECT * FROM hoa_visit_stops WHERE visit_id=$1 ORDER BY sort_order,id`,[visit.id])).rows;res.json(visit);}catch(e){res.status(500).json({error:'visit failed'});}});
app.post('/api/hoa/visits/:visitId/stops/:stopId',requireAuth,requireHoa,upload.array('photos',8),async(req,res)=>{try{const visitId=Number(req.params.visitId),stopId=Number(req.params.stopId),row=(await pool.query(`SELECT s.*,v.company_id FROM hoa_visit_stops s JOIN hoa_property_visits v ON v.id=s.visit_id WHERE s.id=$1 AND s.visit_id=$2 AND v.company_id=$3`,[stopId,visitId,req.hoaCompany.id])).rows[0];if(!row)return res.status(404).json({error:'not found'});if(!req.files||!req.files.length)return res.status(400).json({error:'at least one photo required'});const ids=[];for(const file of req.files){const dims=await imageDims(file.path),photoPath=`/uploads/${file.filename}`,cap=(await pool.query(`INSERT INTO captures(user_id,captured_by,photo_path,photo_width,photo_height,note,area_tags,kind) VALUES($1,$2,$3,$4,$5,$6,$7,'note') RETURNING *`,[req.user.id,req.user.name,photoPath,dims&&dims.w,dims&&dims.h,ticketText(req.body.note,4000),['Inspection']])).rows[0],buffer=fs.readFileSync(file.path);ids.push(cap.id);await pool.query(`INSERT INTO capture_evidence(capture_id,user_id,original_sha256,original_bytes,original_name) VALUES($1,$2,$3,$4,$5)`,[cap.id,req.user.id,crypto.createHash('sha256').update(buffer).digest('hex'),buffer.length,ticketText(file.originalname,255)]);}await pool.query(`UPDATE hoa_visit_stops SET capture_ids=capture_ids || $1::integer[],note=$2,status='complete' WHERE id=$3`,[ids,ticketText(req.body.note,4000),stopId]);const remaining=Number((await pool.query(`SELECT COUNT(*) FROM hoa_visit_stops WHERE visit_id=$1 AND status<>'complete'`,[visitId])).rows[0].count);if(!remaining)await pool.query(`UPDATE hoa_property_visits SET status='completed',completed_at=now() WHERE id=$1`,[visitId]);res.json({ok:true,capture_ids:ids,visit_completed:!remaining});}catch(e){console.error('[hoa.visit.stop]',e);res.status(500).json({error:'visit photo failed'});}});
...
// Board reports remain evidence reports: an item is excluded unless it has a
// property photo, and completion claims are shown beside their original view.
app.get('/api/hoa/report',requireAuth,requireHoa,async(req,res)=>{try{
  const vals=[req.hoaCompany.id],where=['i.company_id=$1'];
  if(Number.isInteger(Number(req.query.community_id))){vals.push(Number(req.query.community_id));where.push(`i.community_id=$${vals.length}`);}
  if(HOA_BUDGETS.includes(req.query.budget)){vals.push(req.query.budget);where.push(`i.budget_source=$${vals.length}`);}
  if(req.query.closed!=='1')where.push(`i.status NOT IN('completed','cancelled')`);
  const items=(await pool.query(`SELECT i.*,c.name community_name,c.address community_address FROM hoa_maintenance_items i JOIN hoa_communities c ON c.id=i.community_id WHERE ${where.join(' AND ')} AND (i.capture_id IS NOT NULL OR EXISTS(SELECT 1 FROM hoa_item_photos hp WHERE hp.item_id=i.id)) ORDER BY c.name,CASE i.priority WHEN 'emergency' THEN 0 WHEN 'high' THEN 1 WHEN 'routine' THEN 2 ELSE 3 END,i.created_at`,vals)).rows;
  for(const item of items){item.photos=(await pool.query(`SELECT cap.*,hp.photo_stage FROM hoa_item_photos hp JOIN captures cap ON cap.id=hp.capture_id WHERE hp.item_id=$1 AND cap.photo_path IS NOT NULL ORDER BY hp.created_at`,[item.id])).rows;if(!item.photos.length&&item.capture_id){const original=(await pool.query(`SELECT * FROM captures WHERE id=$1 AND photo_path IS NOT NULL`,[item.capture_id])).rows[0];if(original)item.photos=[{...original,photo_stage:item.photo_stage||'inspection'}];}}
  const format=req.query.doc==='docx'?'docx':'pdf',title=`${req.hoaCompany.name} - Board Photo Maintenance Report`,summary=`${items.length} photo-backed maintenance item${items.length===1?'':'s'}. Every finding below is supported by property photo evidence.`;
  logEvent(req.user.id,'hoa_board_report',{format,items:items.length});
  if(format==='pdf'){
    res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition','attachment; filename="hoa-board-photo-maintenance-report.pdf"');
    const pdf=new PDFDocument({size:'LETTER',margin:48});pdf.pipe(res);pdf.fontSize(19).text(title,{align:'center'}).moveDown(.4).fontSize(10).text(summary,{align:'center'});
    for(const item of items){pdf.addPage();pdf.fontSize(15).text(item.title).fontSize(10).text(`${item.community_name} | ${String(item.priority||'').replaceAll('_',' ')} | ${String(item.status||'').replaceAll('_',' ')}`).moveDown(.4);if(item.description)pdf.text(item.description).moveDown(.5);const before=item.photos.find(p=>['initial','inspection'].includes(p.photo_stage))||item.photos[0],after=[...item.photos].reverse().find(p=>['completed_work','final_verification'].includes(p.photo_stage)),entries=after&&after.id!==before.id?[['ORIGINAL CONDITION',before,48],['COMPLETED / VERIFIED',after,310]]:[['PHOTO EVIDENCE',before,48]],top=pdf.y;for(const [label,photo,x] of entries){pdf.fontSize(10).text(label,x,top,{width:240});const image=localPhoto(photo.photo_path);if(image){const rendered=await renderForEmbedStamped(image,'standard','jpeg',photo);if(rendered)try{pdf.image(rendered.buffer,x,top+16,{fit:[entries.length===2?240:500,entries.length===2?180:330]});}catch(e){}}}pdf.y=top+(entries.length===2?210:360);pdf.x=48;pdf.fontSize(9).text(`Area: ${item.area||'Not specified'} | Budget: ${String(item.budget_source||'unassigned').replaceAll('_',' ')} | Board approval: ${String(item.board_approval||'not_required').replaceAll('_',' ')}`);}
    pdf.end();return;
  }
...
app.get('/api/hoa/items/:id',requireAuth,requireHoa,async(req,res)=>{try{const id=Number(req.params.id),item=(await pool.query(`SELECT i.*,c.name community_name,c.address community_address,cap.photo_path,cap.photo_title,cap.address photo_address,cap.latitude photo_latitude,cap.longitude photo_longitude FROM hoa_maintenance_items i JOIN hoa_communities c ON c.id=i.community_id LEFT JOIN captures cap ON cap.id=i.capture_id WHERE i.id=$1 AND i.company_id=$2`,[id,req.hoaCompany.id])).rows[0];if(!item)return res.status(404).json({error:'not found'});const history=(await pool.query(`SELECT h.*,u.name user_name FROM hoa_item_history h LEFT JOIN users u ON u.id=h.user_id WHERE h.item_id=$1 ORDER BY h.created_at`,[id])).rows;let photos=(await pool.query(`SELECT c.id,c.photo_path,c.photo_title,c.note,c.address,c.latitude,c.longitude,c.created_at,p.photo_stage FROM hoa_item_photos p JOIN captures c ON c.id=p.capture_id WHERE p.item_id=$1 ORDER BY p.created_at`,[id])).rows;if(!photos.length&&item.capture_id)photos=[{id:item.capture_id,photo_path:item.photo_path,photo_title:item.photo_title,note:item.description,address:item.photo_address,latitude:item.photo_latitude,longitude:item.photo_longitude,created_at:item.created_at,photo_stage:item.photo_stage}];res.json({item,history,photos,evidence:hoaEvidenceChecklist(photos)});}catch(e){res.status(500).json({error:'item failed'});}});
...
app.post('/api/hoa/items/:id/completion-request',requireAuth,requireHoa,async(req,res)=>{try{const id=Number(req.params.id),item=(await pool.query(`SELECT id,title FROM hoa_maintenance_items WHERE id=$1 AND company_id=$2`,[id,req.hoaCompany.id])).rows[0];if(!item)return res.status(404).json({error:'not found'});const token=crypto.randomBytes(24).toString('base64url'),recipient=ticketText(req.body&&req.body.recipient_name,200),expires=new Date(Date.now()+14*86400000);await pool.query(`INSERT INTO hoa_completion_photo_requests(item_id,token,recipient_name,expires_at,created_by) VALUES($1,$2,$3,$4,$5)`,[id,token,recipient,expires,req.user.id]);await hoaHistory(id,req.user.id,'completion_photo_requested',{recipient_name:recipient});res.json({url:`${req.protocol}://${req.get('host')}/completion-photos/${token}`,expires_at:expires});}catch(e){res.status(500).json({error:'request failed'});}});
...
app.get('/completion-photos/:token',async(req,res)=>{try{const row=(await pool.query(`SELECT q.*,i.title,i.description,c.name community_name,cap.photo_path original_photo FROM hoa_completion_photo_requests q JOIN hoa_maintenance_items i ON i.id=q.item_id JOIN hoa_communities c ON c.id=i.community_id LEFT JOIN captures cap ON cap.id=i.capture_id WHERE q.token=$1`,[req.params.token])).rows[0];if(!row)return res.status(404).send('Photo request not found.');if(row.status!=='open'||new Date(row.expires_at)<new Date())return res.status(410).send('This photo request is no longer open.');res.send(`<!doctype html><html><head><meta name="viewport" content="width=device-width"><title>Submit Completion Photos</title><style>body{font-family:Arial,sans-serif;max-width:720px;margin:auto;padding:20px;color:#111}img{width:100%;max-height:480px;object-fit:contain;border-radius:10px;background:#eee}input,textarea,button{box-sizing:border-box;width:100%;padding:14px;margin:7px 0;font-size:16px}button{background:#2455d9;color:#fff;border:0;border-radius:9px;font-weight:700}.box{background:#eef3ff;border-radius:10px;padding:14px}</style></head><body><h1>Submit Completion Photos</h1><div class="box"><b>${escXml(row.community_name)}</b><h2>${escXml(row.title)}</h2><p>${escXml(row.description||'')}</p></div>${row.original_photo?`<h3>Original issue</h3><img src="${escXml(row.original_photo)}" alt="Original maintenance issue">`:''}<h3>Show the completed work</h3><p>Take clear overview and close-up photos. These photos will be added directly to this maintenance record.</p><form method="post" action="/completion-photos/${row.token}" enctype="multipart/form-data"><input name="submitter_name" value="${escXml(row.recipient_name||'')}" placeholder="Your name" required><input type="file" name="photos" accept="image/*" capture="environment" multiple required><textarea name="note" placeholder="What work was completed?"></textarea><button>Send Completion Photos</button></form></body></html>`);}catch(e){res.status(500).send('Photo request unavailable.');}});
app.post('/completion-photos/:token',upload.array('photos',8),async(req,res)=>{try{const q=(await pool.query(`SELECT q.*,i.area,i.company_id FROM hoa_completion_photo_requests q JOIN hoa_maintenance_items i ON i.id=q.item_id WHERE q.token=$1 AND q.status='open' AND q.expires_at>now()`,[req.params.token])).rows[0];if(!q||!req.files||!req.files.length)return res.status(400).send('This request is invalid or no photos were selected.');for(const file of req.files){const dims=await imageDims(file.path),photoPath=`/uploads/${file.filename}`,name=ticketText(req.body.submitter_name,200)||q.recipient_name||'Outside submitter',cap=(await pool.query(`INSERT INTO captures(user_id,captured_by,photo_path,photo_width,photo_height,note,area_tags,kind) VALUES($1,$2,$3,$4,$5,$6,$7,'note') RETURNING *`,[q.created_by,name,photoPath,dims&&dims.w,dims&&dims.h,ticketText(req.body.note,4000),[q.area]])).rows[0],buffer=fs.readFileSync(file.path);await pool.query(`INSERT INTO capture_evidence(capture_id,user_id,original_sha256,original_bytes,original_name) VALUES($1,$2,$3,$4,$5)`,[cap.id,q.created_by,crypto.createHash('sha256').update(buffer).digest('hex'),buffer.length,ticketText(file.originalname,255)]);await pool.query(`INSERT INTO hoa_item_photos(item_id,capture_id,photo_stage) VALUES($1,$2,'completed_work')`,[q.item_id,cap.id]);}await pool.query(`UPDATE hoa_completion_photo_requests SET status='submitted',submitted_at=now() WHERE id=$1`,[q.id]);await hoaHistory(q.item_id,null,'completion_photos_submitted',{submitted_by:ticketText(req.body.submitter_name,200),photo_count:req.files.length});await hoaNotifyCompany(q.company_id,q.item_id,`Completion photos submitted`,0);res.send('<!doctype html><meta name="viewport" content="width=device-width"><style>body{font-family:Arial;text-align:center;padding:50px 20px}h1{color:#2455d9}</style><h1>Photos received</h1><p>Thank you. The completion photos were added to the maintenance record.</p>');}catch(e){console.error('[hoa.completion.submit]',e);res.status(500).send('Photos could not be submitted.');}});
...
function pavingEvidenceChecklist(captures,tickets,pairRows){const paired=new Set();for(const p of pairRows){paired.add(p.before_id);paired.add(p.after_id);}return evidenceChecklist([
  {key:'photos',label:'Job has photo evidence',complete:captures.length>0,warning:'Add at least one job photo.'},
  {key:'notes',label:'Every job photo has a note',complete:captures.length>0&&captures.every(c=>String(c.note||'').trim()),warning:'Add notes to every job photo.'},
  {key:'locations',label:'Every job photo has an address or GPS',complete:captures.length>0&&captures.every(c=>c.address||(c.latitude!=null&&c.longitude!=null)),warning:'Add an address or GPS location to every job photo.'},
  {key:'before_after',label:'Before and after photos are paired',complete:paired.size>=2,warning:'Pair at least one before photo with its after photo.'},
  {key:'completion',label:'Completed work is photographed',complete:captures.some(c=>/complete|completed|finish|after/i.test(String(c.note||'')+' '+String(c.area_tags||'')))||paired.size>=2,warning:'Add a completed-work photo.'},
  {key:'tickets',label:'Delivery ticket evidence is linked',complete:tickets.length>0,warning:'Link at least one delivery ticket to this photographed job.'}
]);}
app.get('/api/paving/jobs/:id/completeness',requireAuth,async(req,res)=>{try{if(await currentProduct(req.user.id)!=='paving')return res.status(403).json({error:'Paving Pro required'});const jobId=Number(req.params.id),job=(await pool.query(`SELECT id FROM jobs WHERE id=$1 AND user_id=$2`,[jobId,req.user.id])).rows[0];if(!job)return res.status(404).json({error:'job not found'});const captures=(await pool.query(`SELECT * FROM captures WHERE user_id=$1 AND job_id=$2 AND photo_path IS NOT NULL`,[req.user.id,jobId])).rows,tickets=(await pool.query(`SELECT * FROM asphalt_tickets WHERE user_id=$1 AND job_id=$2 AND status='saved'`,[req.user.id,jobId])).rows,pairs=(await userPairs(req.user.id)).filter(p=>captures.some(c=>c.id===p.before_id||c.id===p.after_id));res.json(pavingEvidenceChecklist(captures,tickets,pairs));}catch(e){res.status(500).json({error:'completeness failed'});}});
...
app.post('/api/captures/:id/crop', requireAuth, async (req, res) => {
  try {
    ...
    const croppedName = `${crypto.randomUUID()}${ext}`;
    const croppedPath = path.join(UPLOAD_DIR,croppedName);
    fs.writeFileSync(croppedPath, out);

    const d = await imageDims(croppedPath);
    await pool.query(
      `UPDATE captures SET photo_original_path = $1, photo_width = $2, photo_height = $3, photo_path = $6 WHERE id = $4 AND user_id = $5`,
      [originalPath, d ? d.w : null, d ? d.h : null, id, req.user.id, `/uploads/${croppedName}`]);
...
function applyStructuredWordTemplate(generatedBuffer, templatePath, values) {
  const local=localPhoto(templatePath);
  if(!local)throw Error('Saved Word template is unavailable');
  return applyTemplate(generatedBuffer,fs.readFileSync(local),values);
}
