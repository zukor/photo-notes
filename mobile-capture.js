...
class MobileCaptureError extends Error {constructor(status,message){super(message);this.status=status;}}
async function beginMobileCapture(req,pool){
 ...
 if(!UUID.test(key)||typeof edition!=='string')throw new MobileCaptureError(400,'Invalid mobile capture identity');
 ...
 try{
  ...
  if(!user||!user.active||!editionAccess(user).includes(edition))throw new MobileCaptureError(403,'This version is no longer authorized for your account');
  const receipt=(await client.query('SELECT request_hash,response FROM mobile_capture_receipts WHERE user_id=$1 AND request_id=$2',[req.user.id,key])).rows[0];
  if(receipt){
   if(receipt.request_hash!==fingerprint)throw new MobileCaptureError(409,'This capture identity was already used for different content');
  ...
  if(currentEdition(user)!==edition)throw new MobileCaptureError(409,'Select the original Photo Notes version before retrying this photo');
  return {db:client,async commit(response){await client.query('INSERT INTO mobile_capture_receipts(user_id,request_id,request_hash,response) VALUES($1,$2,$3,$4)',[req.user.id,key,fingerprint,JSON.stringify(response)]);await client.query('COMMIT');open=false;},async rollback(){if(open){await client.query('ROLLBACK');open=false;}},release(){client.release();}};
 }catch(error){if(open)await client.query('ROLLBACK');client.release();throw error;}
}
function validateMobileCaptureAccount(req,res,next){
...
module.exports={beginMobileCapture,MobileCaptureError,validateMobileCaptureAccount};
