...
test('UI review: owner decisions, tester clarification, approval-only queue and browser controls',{skip:process.env.PN_UI_REVIEW_TEST!=='1',timeout:90000},async()=>{
 ...
 if(type==='bug_problem'){assert.ok(await card.getByRole('heading',{name:'Why This Needs Your Review'}).isVisible());assert.ok((await card.textContent()).includes('saved result does not clearly say'));assert.equal(await card.locator('textarea').count(),1);assert.equal(await card.getByRole('heading',{name:'What Was Fixed',exact:true}).count(),0);assert.equal(await card.locator('summary').filter({hasText:'Technical Details And History'}).count(),1);}
