...
test('UI review: owner decisions, tester clarification, approval-only queue and browser controls',{skip:process.env.PN_UI_REVIEW_TEST!=='1',timeout:90000},async()=>{
 process.env.ISSUE_CLOUD_RUNNER_ENABLED='false';process.env.DATABASE_URL='postgresql://127.0.0.1:55489/pn_pro_retest';process.env.PGSSL='disable';process.env.SESSION_SECRET='ui-review-local';process.env.TESTER_QUEUE_TOKEN='local-ui-review-queue';
 ...
 let browser;try{
 const report=(await pool.query("INSERT INTO issue_reports(user_id,issue_type,description,page_name) VALUES($1,'ui_improvement',$2,'Organize') RETURNING *",[users[2],'What happened: Original suggestion\nWhat went wrong?\nToo much text\nWhat did you expect to happen?\nClear controls\nRecommended improvement:\nSimplify\nExpected result:\nLess clutter\nFrequency: Every time'])).rows[0];
 ...
 const queue=await(await worker('/api/automation/testing-queue')).json();assert.equal(queue.issues.find(i=>i.id===id).implementation_instructions,'Use compact controls; retain all actions.');assert.equal(current.issue_type,'ui_improvement');
