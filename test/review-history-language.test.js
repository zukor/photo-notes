...
test('photo cards and history panel use plain history language',()=>{
  assert.match(app,/>Photo Details &amp; History<\/button>/);
  assert.match(app,/textContent='Photo Details & History'/);
  assert.match(app,/>Technical file details<\/summary>/);
  assert.match(app,/>Changes<\/h3>/);
  assert.doesNotMatch(app,/>Photo History<\/button>/);
...
test('Photo Details & History shows the format detected from the current saved file',()=>{
